...
function DrDamage:ACTIONBAR_SLOT_CHANGED(_, id)
	...
	elseif shaman and (id == 133 or id == 134 or id == 135 or id == 136) then
		if not self.fullUpdate then
			self:ScheduleTimer("UpdateAB", 0.25, nil, id)
		end
	end
end
	...
	if name then
		...
		if settings.DisplayType2 == "Casts" then
			...
			if manaCost and (manaCost > 0) and powerType == 0 then
				...
				manatext = DrD_CreateText(button, true, text, nil, nil, nil, r, g, b)
			end
		...
		if ManaCost or PowerCost then
			...
			if manaCost and (manaCost > 0) and (ManaCost and powerType == 0 or PowerCost and (playerMelee and powerType == 0 or powerType == 1 or powerType == 3)) then
				...
				manatext = DrD_CreateText(button, true, manaCost, nil, nil, nil, r, g, b)
		...
		if spellInfo[name] then
			...
			if baseSpell then
				...
				if text2 then
					text2 = DrD_CreateText(button, true, text2, nil, healingSpell, nil, r, g, b)
				end
				return DrD_CreateText(button, nil, text, macro, healingSpell), text2 or manatext
			end
		end
	end
end
...
function DrDamage:GetMitigation( armor )
	local playerLevel = UnitLevel("player")
	if playerLevel < 60 then
		return math_min(0.75, armor / (armor + 400 + 85 * playerLevel))
	else
